...
24시간: 다른 사람이 PR 작성 권한을 선점할 수 있는 기간

48시간: 책임자가 댓글 단 후 이 시간이 지나면 다른 사람이 PR 작성 우선권을 가져갈 수 있음

문서:

24시간 이내에 처리 권한이 유지됩니다.

이슈 템플릿 & 레이블

이슈를 작성할 때 올바른 템플릿(bug_report, feature_request, document-request)을 선택하면 자동으로 레이블이 붙습니다.

레이블을 수동 변경하지 마세요. 잘못 변경된 경우 이슈가 반려될 수 있습니다.

PR 작성 시

이슈 레이블은 자동으로 달리지 않습니다. PR에 이슈와 동일한 레이블을 수동으로 달아 주세요.

PR 본문 최상단에 “참조 이슈 #번호” 링크를 반드시 붙여넣어야 합니다.

동시 작업 제한

한 저장소에서 개인당 동시에 작업할 수 있는 이슈는 최대 1개입니다.

다른 저장소(reposcore-js 등)에서는 별도로 1개씩 추가로 작업 가능합니다.

이슈 작성 가이드

가능한 한 구체적으로 문제를 기술하세요.

“어떤 파일의 어떤 부분”을 수정해야 하는지, “해결 후 기대 결과”를 명시하면 더욱 좋습니다.
